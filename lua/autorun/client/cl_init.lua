...
function draw.RoundedPolyBoxEX( radius, x, y, w, h, prec, u, v, tl, tr, bl, br, cache )
	if ( prec <= 0 or radius <= 0 or w <= 0 or h <= 0 ) then
...
function draw.RoundedPolyBorderEX( radius, extrude, x, y, w, h, prec, u, v, u_offset, v_offset, tl, tr, bl, br, cache )
	if ( extrude == 0 or prec <= 0 or radius <= 0 or w <= 0 or h <= 0 ) then
		if !cache then return end
		return {}
	end
	...
	--Outer points of bridges
	local x1_1, x1_2, y3_1, y3_2, x5_1, x5_2, y7_1, y7_2 = p1_x, p2_x, p1_y, p2_y, p2_x, p1_x, p2_y, p1_y
	...
	local inner, outer = {}, {} --Initialize verts for the shape.
	--For V wrapping
	local t_length, r_length, b_length, l_length = x1_2 - x1_1, y3_2 - y3_1, x5_1 - x5_2, y7_1 - y7_2
	local total_length = t_length + r_length + b_length + l_length + ( pi_2 * radius + extrude ) * rounded_corners
	local corner_v = ( ( pi_2 * radius + extrude ) / total_length ) / prec

	total_length = 1 / total_length --Inverse
	local v_info = { t_length * total_length, r_length * total_length, b_length * total_length, l_length * total_length } -- V values of bridges
	...
	for case = 1, 4 do
		if corners[case] then --making a rounded corner
			...
			for i = 0, prec do
				if i == 0 then v_seg = v_info[case] else v_seg = corner_v end
				local a = (( i / prec ) * -pi_2) - (pi_2 * (case - 1))

				inner[#inner + 1] = { x = p_x - sin( a ) * radius, y = p_y - cos( a ) * radius, u = u_offset, v = v_seg * v + real_estate }
				outer[#outer + 1] = { x = inner[#inner].x - (sin( a ) * extrude), y = inner[#inner].y - (cos( a ) * extrude ), u = u + u_offset, v = v_seg * v + real_estate }
				real_estate = v_seg * v + real_estate --Update

				if extrude < 0 then inner[#inner].u, outer[#outer].u = outer[#outer].u, inner[#inner].u end --need to be swapped
				if i != 0 or case != 1 then --only drawpoly when we have 4 points
					local quad
					if extrude > 0 then
			...
			if extrude < 0 then inner[#inner].u, outer[#outer].u = outer[#outer].u, inner[#inner].u end
	...
	local in_begin, out_begin = { x = inner[1].x, y = inner[1].y, u = u_offset, v = v + inner[1].v }, { x = outer[1].x, y = outer[1].y, u = u + u_offset, v = v + outer[1].v }

	local quad
	if extrude < 0 then
		in_begin.u, out_begin.u = out_begin.u, in_begin.u
